<div class="mx-auto max-w-2xl px-4 py-12 sm:px-6 lg:px-8">
    ...
    <div class="space-y-6">
        ...
        <div class="rounded-2xl border border-gray-200 bg-white px-6 py-5 shadow-md shadow-gray-100">
            ...
            <div class="grid gap-3 {{ $providers->count() > 2 ? 'grid-cols-3' : 'grid-cols-2' }}">
                @foreach($providers as $provider)
                    @php $isSelected = $selectedProviderId === $provider->id; @endphp
                    <label class="relative flex cursor-pointer flex-col items-center gap-2 rounded-xl border p-4 text-center transition-all
                        {{ $isSelected
                        ...
                        @if($provider->logo_path)
                            <img
                                src="{{ Storage::disk('r2')->url($provider->logo_path) }}"
                                alt="{{ $provider->name }}"
                                class="h-10 w-auto max-w-[72px] object-contain"
                            >
                        @else
                        ...
                        <span class="text-xs font-semibold {{ $isSelected ? 'text-brand-700' : 'text-gray-700' }}">
                            {{ $provider->name }}
                        </span>
                        ...
                        @if($isSelected)
                            <span class="absolute right-2 top-2">
                                <svg class="h-4 w-4 text-brand-600" fill="currentColor" viewBox="0 0 20 20">
        ...
        {{-- M-Pesa: phone number --}}
        @if($selectedProvider?->slug === 'mpesa')
        <div class="rounded-2xl border border-gray-200 bg-white px-6 py-6 shadow-md shadow-gray-100">
            <div class="mb-4 flex items-center gap-3">
                @if($selectedProvider->logo_path)
                    <img src="{{ Storage::disk('r2')->url($selectedProvider->logo_path) }}" alt="M-Pesa" class="h-7 w-auto object-contain">
                @endif
                <h2 class="text-sm font-semibold uppercase tracking-wide text-gray-500">M-Pesa payment</h2>
            </div>
        ...
        <button
            ...
            wire:target="pay"
            @if(!$selectedProviderId) disabled @endif
            class="w-full inline-flex items-center justify-center rounded-full bg-accent-600 h-10 px-5 text-sm font-semibold text-white hover:bg-accent-700 transition disabled:opacity-50 disabled:cursor-not-allowed"
        >
            <span class="whitespace-nowrap">
                @if($selectedProvider)
                    Pay {{ $currency }} {{ number_format($total, 2) }} via {{ $selectedProvider->name }}
                @else
        ...
        @elseif($selectedProvider)
            <p class="text-center text-xs text-gray-400">
                Payments are processed securely via {{ $selectedProvider->name }}.
            </p>
        @endif
